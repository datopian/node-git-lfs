...
const AUTHENTICATOR = Authenticator.getAuthenticator(
  config.get("authenticator.type"),
  config.get("authenticator.options")
);
  ...
  app.post(
    ...
    wrap(function*(req, res, next) {
      ...
      try {
        ...
        req.jsonBody = body;
        var validatorResult = validate(body, BATCH_REQUEST_SCHEMA);
        var valid = validatorResult.valid;
        if (!valid) {
          console.error(validatorResult.toString());
          let err = new Error();
          err.status = 422;
          next(err);
        } else {
      ...
      try {
        ...
        switch (operation) {
          case "upload":
            ...
            let canWrite = yield AUTHENTICATOR.canWrite(
              user,
              repo,
              authorization
            );
